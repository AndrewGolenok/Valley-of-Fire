...
for(i=1;i<=6;i++)
	...
	if global.player_rank <= 3
		{ hand_count = 4; hand_red[i] = choose(0, 1); min_time[3,3] = 80; }
	if global.player_rank < 7  && global.player_rank >= 4
		{ hand_count = 4; hand_red[i] = choose(0, 0, 1); min_time[3,3] = 60; }
	if global.player_rank < 10 && global.player_rank >= 7
		{ hand_count = 3; hand_red[i] = choose(0, 0, 0, 1); min_time[3,3] = 60; }
	if global.player_rank < 13 && global.player_rank >= 10
		{ hand_count = 3; hand_red[i] = choose(0, 0, 0, 0, 1); min_time[3,3] = 50; }
	if global.player_rank < 15 && global.player_rank >= 13
		{ hand_count = 2; min_time[3,3] = 40; }
	if global.player_rank >= 15
		{ hand_count = 1; min_time[3,3] = 20; }
