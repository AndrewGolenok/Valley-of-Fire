with(o_hero)
	...
	if !enemy
		...
		if o_list.e_hp - o_list.atk <= 0
			{ datk = (1 + 0.2 * global.e_totem_a[6]) }
		
		if miss = 0
			{
			if o_list.hp > 0 && o_list.e_hp > 0
				{ o_list.hp -= o_list.e_atk * datk + global.e_totem_a[5] * o_list.e_atk / 10 * (global.e_critical == (3 - 2 * global.e_totem_a[3])); }
			}
		...
		if diego_dynamit = 1 && skeleton_animation_get() != "super"
			{ dynamit_d = o_list.e_atk + global.e_totem_a[5] * o_list.e_atk / 10 * (global.e_critical == (3 - 2 * global.e_totem_a[3])); diego_dynamit = 2; stun_seconds -= 1; change = 3; skeleton_animation_set("super"); }
							
		o_list.dop_i[1] = 7;
		o_list.dop_text_color[1] = (global.enemy_object).hero_color;
		if global.critical == (3 - 2 * global.e_totem_a[3])
			{ o_list.dop_text[7] = "-" + string(round(o_list.e_atk + global.e_totem_a[5] * o_list.e_atk / 10)); }
			else
